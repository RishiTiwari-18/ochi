...
const Nav = () => {
  const navItems = ["Services", "Our Work", "About us", "Insights", "Contact us"]
  ...
  return (
    <nav 
      ref={navRef} 
      className='fixed w-full z-[999] px-14 py-4 flex items-center justify-between backdrop-blur-sm  '
    >
        <div className="logo">
        ...
        <div className='flex items-center gap-7 max-sm:gap-0 '>
            {navItems.map((item, index) => ((
                <a href="#" key={index} className={`${index ===4 && "ml-72"} text-[16.6px] font-normal tracking-tight `}>{item}</a>
            )))}
        </div>
    </nav>
  )
